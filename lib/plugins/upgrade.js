...
var upgrade = function(req, next, abort) {
    ...
    upgrade.getService(req.argv._[0])
            .then(deploy.upload(config.remote))
...
upgrade.getService = function(reference) {
    ...
    return new Promise(function(resolve, reject) {  
        ...
        request.get({
            url: config.endpoint + '/v1/services/' + reference + '?api_token=' + token,
            json: true,
...
upgrade.launch = function(masonJson) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        // Deploy
        request.put({
            url: config.endpoint + '/v1/services/' + upgrade.service.id + '?api_token=' + token,
            json: true,
            form: masonJson 
        }, function(error, response, body) {
