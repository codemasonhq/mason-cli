...
deploy.prepareMasonJson = function() {
    return new Promise(function(resolve, reject) {
        ...
        if(_.has(masonJson, 'app.build')) {

            // Remove build, use image
            delete masonJson.app.build;
            delete masonJson.app.volumes; 

            // Set image as repo's image
            masonJson.app.image = deploy.config.registry + "/" + deploy.config.project;

            // Load environment file, if available 
            if(helpers.fileExists('.env')) {
                masonJson.app.environment = _.merge(masonJson.app.environment, env('.env'));
            }
    ...
    return new Promise(function(resolve, reject) {
        ...
        var requests = _.map(masonJson, function(service) {
            return new Promise(function(resolve, reject) {
                ...
                request.post({
                    url: deploy.config.endpoint + '/v1/services?application=' + deploy.config.application + '&api_token=' + token,
                    json: true,
                    form: service
                }, function(error, response, body) {
