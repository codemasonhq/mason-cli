...
deploy.prepareMasonJson = function() {
    return new Promise(function(resolve, reject) {
        masonJson = {
            ...
            type: "service",
            masonVersion: "v1",
            image: config.registry + "/" + config.project,
            environment: {
                "APP_ENV": "local",
...
deploy.launch = function(masonJson) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        // Deploy
        request.post({
            url: config.endpoint + '/v1/services?application=' + config.application + '&api_token=' + token,
            json: true,
            form: masonJson 
        }, function(error, response, body) {
