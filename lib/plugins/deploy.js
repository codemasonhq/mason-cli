...
var deploy = function(req, next, abort) {
    ...
    // FIX ME: Dynamically get this from the git remote
    config.namespace = "root";
    config.path = "tested-heat-3430";
    config.project = config.namespace + "/" + config.path;
...
deploy.getBuild = function(project, build) {
    ...
    return new Promise(function(resolve, reject) {  
        ...
        request.get({
            ...
            if(error || body.errors != undefined) {
                helpers.log("   ✗ Could not access build trace".red);
                return;
            }
    ...
    return new Promise(function(resolve, reject) {
        masonJson = {
            ...
            services: [
                {
                    name: "nginx",
                    image: "docker:codemasonhq/nginx",
                    type: "service",
                    ports: [
                        "80",
                        "443"
...
deploy.launch = function(masonJson) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        // Deploy
        request.post({
            url: config.endpoint + '/v1/deploy?api_token=' + token,
            json: true,
            form: masonJson 
        }, function(error, response, body) {
