...
var craft = function(req, next, abort) {
    ...
    var dockerfile = craft.compileDockerfile();

    // Add docker-compose.yml file to app
    var composefile = craft.compileDockerCompose();

    // Add .gitlab-ci.yml file to app
    var gitlabcifile = craft.compileGitLabCI();
...
craft.compileDockerfile = function() {
    ...
    // Write
    craft.writeFile('Dockerfile', result);
...
craft.compileDockerCompose = function() {
    ...
    // Write
    craft.writeFile('docker-compose.yml', result);
...
craft.compileGitLabCI = function() {
    ...
    // Write
    craft.writeFile('.gitlab-ci.yml', result);
...
craft.writeFile = function(filename, contents) {
    try {
        var foo = path.join(process.cwd(), filename);
        fs.writeFileSync(filename, contents, { encoding: 'utf8' });
        helpers.log(` + Wrote ${ filename }`);
    }
    catch (e) {
